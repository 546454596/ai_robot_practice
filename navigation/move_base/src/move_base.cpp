...
namespace move_base {
  ...
  MoveBase::MoveBase(tf2_ros::Buffer& tf) :
    ...
    planner_plan_(NULL), stop_inspect_(false), latest_plan_(NULL), controller_plan_(NULL),
...
//    private_nh.param("base_global_planner", global_planner, std::string("navfn/NavfnROS"));
    ...
    vel_pub_ = nh.advertise<geometry_msgs::Twist>("cmd_vel_cx", 1);
    current_goal_pub_ = private_nh.advertise<geometry_msgs::PoseStamped>("current_goal", 0 );
    ros::NodeHandle action_nh("move_base");
    action_goal_pub_ = action_nh.advertise<move_base_msgs::MoveBaseActionGoal>("goal", 1);
    path_pub = nh.advertise<nav_msgs::Path>("/topo/path", 1);
    recovery_status_pub_= action_nh.advertise<move_base_msgs::RecoveryStatus>("recovery_status", 1);
    //we'll provide a mechanism for some people to send goals as PoseStamped messages over a topic, they won't get any useful information back about its status, but this is useful for tools like nav_view and rviz
    ros::NodeHandle simple_nh("move_base_simple");
    goal_sub_ = simple_nh.subscribe<geometry_msgs::PoseStamped>("goal_cx", 1, boost::bind(&MoveBase::goalCB, this, _1));
    stop_inspect = nh.subscribe<std_msgs::Bool>("/obs_abort", 1, boost::bind(&MoveBase::stop_inspecCB, this, _1));
//    path_sub = nh.subscribe("/topo/path", 1, &MoveBase::pathCB, this);
    ...
    planner_->initialize("NavfnROS", planner_costmap_ros_);
...
//      tc_.reset();
  ...
  void MoveBase::goalCB(const geometry_msgs::PoseStamped::ConstPtr& goal){
   ...
   void MoveBase::stop_inspecCB(const std_msgs::BoolConstPtr& msg)
   {
     stop_inspect_ = msg->data;
   }
  ...
  void MoveBase::planThread(){
    ...
    while(n.ok()){
      ...
      while(wait_for_wake || !runPlanner_){
        ...
        planner_cond_.wait(lock);//notify_one()向其中一个正在等待的线程发送“条件成立”的信号，  wait()阻塞当前线程，等待条件成立
        wait_for_wake = false;
      }
      ros::Time start_time = ros::Time::now();
      //time to plan! get a copy of the goal and unlock the mutex
      geometry_msgs::PoseStamped temp_goal = planner_goal_;//planner_goal_为executCb中获得的map frame下的全局目的地
      lock.unlock();
      //run planner
      planner_plan_->clear();
      gotPlan = n.ok() && makePlan(temp_goal, *planner_plan_);//planner_plan_存储了全局规划的path
      if(gotPlan){
        ROS_DEBUG_NAMED("move_base_plan_thread","Got Plan with %zu points!", planner_plan_->size());
        //pointer swap the plans under mutex (the controller will pull from latest_plan_)
//        path_msg.poses = *planner_plan_;
//        path_pub.publish(path_msg);
        std::vector<geometry_msgs::PoseStamped>* temp_plan = planner_plan_;//planner_plan_赋值给temp_plan,temp_plan赋值给latest_plan
        lock.lock();
        planner_plan_ = latest_plan_;
        latest_plan_ = temp_plan;//latest_plan赋值给controller_plan in executeCb函数中
      ...
      //if we didn't get a plan and we are in the planning state (the robot isn't moving)
        ...
        //check if we've tried to make a plan for over our time limit or our maximum number of retries //issue #496: we stop planning when one of the conditions is true, but if max_planning_retries_
  ...
  //goal is the goal_pose in map frame，将全局规划的轨迹存储在plan中
  bool MoveBase::makePlan(const geometry_msgs::PoseStamped& goal, std::vector<geometry_msgs::PoseStamped>& plan){
    ...
    geometry_msgs::PoseStamped global_pose;
    if(!getRobotPose(global_pose, planner_costmap_ros_)) {//global_pose为全局坐标系下机器人位姿
  ...
  bool MoveBase::executeCycle(geometry_msgs::PoseStamped& goal){
    ...
    if(new_global_plan_){//在planThread中置true
      ...
      boost::unique_lock<boost::recursive_mutex> lock(planner_mutex_);
      controller_plan_ = latest_plan_;//在planThread中获得latest_plan_，存储了全局规划path,这里将其赋值给controller_plan_
      path_msg.header = controller_plan_->back().header;
      path_msg.poses = *controller_plan_;
      path_pub.publish(path_msg);
      latest_plan_ = temp_plan;
      lock.unlock();
      ROS_DEBUG_NAMED("move_base","pointers swapped!");
/*      if(!tc_->setPlan(*controller_plan_)){//controller_plan_存储了全局规划路路径点（map为frame），在局部规划器中进行局部规划
